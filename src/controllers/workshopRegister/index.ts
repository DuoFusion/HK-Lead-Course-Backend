...
export const addWorkShopRegister = async (req, res) => {
    ...
    try {
        ...
        let order = new workshopRegisterModel(body);
        await order.save();
        ...
        order = await workshopRegisterModel.findOneAndUpdate({ _id: new ObjectId(order._id) }, { razorpayOrderId: razorpayOrder.id }, { new: true });
...
export const verifyRazorpayPayment = async (req, res) => {
    ...
    try {
        ...
        const expectedSignature = crypto.createHmac("sha256", user.razorpayKeySecret)
            .update(sign)
            .digest("hex");
