    ...
    try {
        ...
        if (!_id) {
            return res.status(400).json(
                new apiResponse(400, "Cupan ID is required", {}, {})
            );
        }

        const response = await cupanCodeModel.findOneAndUpdate(
            { _id: new mongoose.Types.ObjectId(_id) }, // ✅ Safe ObjectId conversion
            { $set: updateFields },
            { new: true, lean: true }
        );
        ...
        if (!response) {
            return res.status(404).json(
                new apiResponse(404, responseMessage.getDataNotFound("Cupan"), {}, {})
            );
        }

        return res.status(200).json(
            new apiResponse(200, responseMessage.updateDataSuccess("Cupan"), response, {})
        );
